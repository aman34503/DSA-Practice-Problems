...
public:
    ...
    int maximumprofit(vector<int>& prices, int currentday, int canbuy, int transcount){
        if(currentday >= prices.size() || transcount == 0){
            return 0;
        }
        
        if(canbuy == 1){
            int idle = maximumprofit(prices, currentday+1, canbuy, transcount);
            int buy = -prices[currentday] + maximumprofit(prices, currentday+1, 0, transcount);
            return max(idle, buy);
    }
        else{
            int idle = maximumprofit(prices, currentday+1, canbuy, transcount);
            int sell = prices[currentday] + maximumprofit(prices, currentday+1, 1, transcount-1);
